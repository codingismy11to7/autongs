...
object OptionsDialog {
  ...
  final val Component = ScalaFnComponent
    ...
    .custom(isBulkBuying)
    .render {
      (props, controller, started, savedOptions, currOptions, sendNotif, currNotif, machinesDlgOpen, bulkBuying) =>
        ...
        val onToggleBulkBuy = controller.toggleBulkBuy *> handleClose
          ...
          MuiDialog[RTask](
                ...
                MuiTooltip[RTask](title = "Instead of Auto-EMCing only on Dyson Segments, do it on every page.").apply(
                  MuiFormControlLabel[RTask](
                    ...
                    control = MuiCheckbox(color = MuiCheckbox.Color.primary)(
                      ...
                      ^.onChange ==> ((e: ReactEventFromInput) =>
                        setCurrOptions(_.copy(emcAllPages = e.target.checked))
                      ),
                    ).rawElement,
                  )
                ),
            ...
            MuiDialogActions()(
              ...
              MuiTooltip[RTask](title =
                ...
                MuiButton(color = if (bulkBuying) MuiButton.Color.secondary else MuiButton.Color.primary)(
                  ^.onClick --> onToggleBulkBuy,
                  s"${if (bulkBuying) "Stop " else ""}Bulk Buy",
                )
              ),
              MuiButton()(
